...
def _():
    ...
    P = load_params()
    return (
        P, acquire, annotate, dataset, db, gpd, model, nuts, osm, pg_engine,
        raster, rio, settings,
    )
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## 2 · Acquisizione Sentinel-2 (STAC + COG)""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## 4 · Tile + Label YOLO""")
    return


@app.cell
def _(P, annotate, raster, rio):
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## 5 · Dataset + 6 · Training""")
    return


@app.cell
def _(P, dataset, model, tiles_dir):
